...
(() => {
  ...
  function App() {
    ...
    const [apiBase, setApiBase] = useState2(localStorage.getItem("API_BASE_URL") || "");
    ...
    function saveApiBase() {
      ...
      setApiBase(clean);
      call.loadRtcConfig().then(() => notify("API Base URL saved")).catch(() => notify("API Base URL saved (RTC defaults loaded)"));
    }
    async function testConnection() {
      try {
        const body = await api("/health");
        if (!(body == null ? void 0 : body.ok)) throw new Error("Health check response invalid");
        await call.loadRtcConfig();
